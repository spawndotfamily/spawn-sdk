...
const names: Record<string, string> = { alice: 'Alice', bob: 'Bob', empty: 'Empty balance', creator: 'Creator wallet', pool: 'Game pool' };

/** Operator controls belong to the launcher, not to the untrusted game frame. */
export function createCreatorPanel(getState: () => LocalTestState, getPlayer: () => string) {
  ...
  function refresh() {
    ...
    for (const item of state.economy.history.slice(0, 12)) {
      ...
      const route = document.createElement('span'); route.textContent = `${names[item.from]} → ${names[item.to]}`;
  ...
  function transfer(kind: 'fund' | 'withdraw' | 'reward') {
    try {
      ...
      feedback.textContent = `${receipt.amount} TEST moved from ${names[receipt.from]} to ${names[receipt.to]}.`;
