...
The documented game entry flow keeps the legacy `requestPayment('entry')` method for compatibility: without a configured token it rejects on current hosted Spawn, while a configured listing represents a permanent game-access purchase and must return an existing purchase receipt or a clear no-purchase-needed error instead of debiting that price again. `requestTokenPayment({ amount, item? })` requires an explicit human-unit decimal amount string and can include an item label for a separate optional payment. Both methods get their asset from the same listing setting; neither accepts a token address, player, recipient or fee. The platform resolves the asset, price, recipient and quote server-side and binds a short-lived intent to the signed-in player who launched that game. Only enabled Spawn-admitted `spawn` or `partner` assets on chain ID 46630 testnet are available. A game-supplied player ID, apparent success screen or sign-in session is not spending permission.

The player confirms on a Spawn-owned page with a visible browser origin, exact total, asset, recipient game, the 0% fee policy for new game payments and projected remaining balance. A ten-token approval cannot be changed into a 1,000-token charge or reused for another entry. The game cannot frame or control the real confirmation page, and its credentials cannot call the player-confirmation endpoint. A copied imitation cannot authorize a debit.

The local approval host uses isolated synthetic balances. Hosted private previews use the configured Listing testnet token and still require real player confirmation; local credentials and fabricated receipts must fail against hosted services. Never add a game-side confirmation bypass. On cancellation or expiry, grant no paid entitlement. A timeout can happen after payment committed: use the server-only `createSpawnPaymentClient().lookup()` with the original saved player, launch and request IDs, as documented in [token purchases](token-payments.md). A missing result is unresolved, not permission to charge again. The game server checks the authoritative receipt against its saved catalog order and atomically records a unique receipt claim together with item delivery in its own database. A browser receipt is a lookup hint, not payment authority or proof of honest gameplay.

Real redeemable SDK payments are not enabled. Signed multiplayer launch proof is a distinct account-admission contract, not payment permission. Test-token entries are non-redeemable and do not authorize real charges. Do not invent SDK payment methods or enable real charges to simulate this flow.

### Overlay and receipt ownership

The platform flow is Confirm → Processing → Paid checkmark → Continue. Payment UI and receipt history are Spawn-owned, outside the game. Real-money transfers remain unavailable. The game requests payment through the SDK and waits for a paid receipt after the platform's confirmation/Continue flow. It must not treat an open overlay, checkmark, local boolean, saved flag or arbitrary postMessage as proof. A duplicate in-game confirmation UI cannot authorize a debit. History must show payer identity from the authenticated platform account to the appropriately authorized player and creator; game-supplied identities are never the source of that record.
...
The local CLI prepares a private preview and can read its status. After the exact release passes its automated malware check and is play-tested, the creator or an explicitly authorized agent may run `spawn-publish release --release <release-id> --creator-confirmation`. The upload command never publishes automatically. The platform rechecks the project owner credential, exact release scan and suspension state; a legacy platform may return `pending_review`. New credentials may include `build:publish`, while existing active project credentials remain server-authorized; changing the local scopes field cannot grant authority. The CLI must never impersonate staff, publish another project or bypass any server gate. Game deletion and distributions remain dashboard operations. Listing detail edits and individual image removal are separate versioned actions and cannot publish a draft.
