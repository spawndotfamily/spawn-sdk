...
The documented game entry flow lets `requestPayment('entry')` use one token and default amount selected in the project's listing settings, or the fixed 10 TEST fallback when no token is configured. `requestTokenPayment({ amount?, item? })` can supply only a human-unit decimal amount string and item label. Both methods get their asset from the same listing setting; neither accepts a token address, player, recipient or fee. The platform resolves the asset, price, recipient and quote server-side and binds a short-lived intent to the signed-in player who launched that game. Only enabled Spawn-admitted `spawn` or `partner` assets on chain ID 46630 testnet are available. A game-supplied player ID, apparent success screen or sign-in session is not spending permission.
