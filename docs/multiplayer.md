...
For a separate optional payment, use `requestTokenPayment({ amount, item?, requestId? })`. `amount` is required and must be a positive exact human-readable decimal string; `item` is an optional 1–80 character printable label, and `requestId` is an optional UUID. The Listing selects the token, so multiplayer game code never supplies a contract, player, recipient or fee. Spawn opens the same trusted token confirmation overlay used by the document client and resolves with `{ id, assetId, amount, projectId, status: 'paid' }` only after the player confirms. Guests cannot pay. Cancellation, bridge disposal or a timeout rejects the call; if the result is uncertain, the authoritative server retains the original request, player and launch tuple and uses the lookup contract below. Never retry automatically or treat a launch ticket as payment proof.

For an item purchase, the authoritative server must create and persist a UUID order before calling this browser method. Bind that order to the player and launch from verified admission, and keep the local catalog's exact item, Listing asset and base-unit amount. After Continue, or when the browser result is missing, use the server-only [`createSpawnPaymentClient.lookup`](token-payments.md) with the original `{ playerId, requestId, launchId }` tuple. A new browser launch can have a different payment-request scope, so it is not recovery for the original order. Compare the exact lookup response and insert the unique receipt claim plus item grant in one creator-database transaction. `not_found` never authorizes a replacement charge, and cancellation never delivers the item.
