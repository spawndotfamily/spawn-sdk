...
**Testing multiplayer without a second person.** Each player's Approve click is simulated with `confirmBuyIn(playerId)`, so you can seat and play with as many players as your game allows, headlessly — `examples/table-multiplayer-sim.mjs` is a worked three-player flow with a short all-in, a side pot and a cash-out. `testing/README.md` states what the simulation cannot stand in for (the real overlay and a real account's authorization), which is why one two-account preview match stays a human step before publishing.
