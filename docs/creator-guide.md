...
| Creator pools | Dashboard controls and server-only payouts to verified members | New game payments and pool transfers are one-to-one with a 0% Spawn platform fee. Historical receipts retain their recorded fee fields. Pool balance is not a guaranteed prize. Use [server payouts](payouts.md) for automated distributions; no general browser payout API. |
...
| Verified multiplayer purchases | Server-side recovery lookup for an exact Listing-token item payment | Read [verified token purchases](token-payments.md). Journal a server-generated order ID bound to the verified player and launch, compare the exact catalog item/asset/amount, then claim the receipt and issue the item in one creator-database transaction. |
...
4. **Test.** Build into any folder, then run `npx --no-install spawn-publish check <folder>`. Use [spawn-test-host](local-approval-testing.md) for actual token approval/ledger tests and `npx --no-install spawn-dev <folder>` separately for saves/scores. Play a real start → gameplay → finish/retry loop. Check input, missing assets, console errors, reconnect, saves and any cancel/confirmed payment (local simulation and hosted Listing-token checks are separate). Return evidence and clearly state anything you could not test.
...
For multiplayer item sales, use the server-only [`createSpawnPaymentClient.lookup`](token-payments.md) recovery contract. Save the server-generated request ID, verified player/launch, exact catalog item, asset and base-unit amount before opening the browser overlay. A paid lookup is only an input to the server's exact comparison and unique receipt-claim transaction; `not_found`, cancellation, expiration, timeout and browser receipts alone never deliver an item.
