...
For multiplayer item purchases, read [verified token purchases](token-payments.md). Journal a server-generated order ID before calling `requestTokenPayment`, bind it to the player and launch from verified server admission, and keep the local item, Listing asset and exact base-unit amount. After Continue or a timeout, use `createSpawnPaymentClient.lookup` with that original tuple. Only an exact `paid` response may enter the creator database transaction that inserts a unique receipt claim and grants the item. `pending`, `cancelled`, `expired`, `not_found` and unavailable reads do not authorize delivery or a replacement debit.

A browser can be modified to bypass a local unlock and cannot prove honest offline scores. Paid leaderboard/reward eligibility must be verified against platform payment records for the authenticated account and game, with entry reuse rules enforced server-side. A payment does not validate a score. The documented lookup in [verified token purchases](token-payments.md) is limited to exact multiplayer purchase verification; it does not grant reward, paid-eligibility or anti-cheat authority. If a broader server eligibility contract is missing, keep paid participation unavailable. Configured testnet token entries are non-redeemable; real-money payments are unavailable.
...
Run the game's meaningful tests and production build. Validate with `spawn-publish check ./dist`, then follow [local testing](testing.md) using `spawn-dev ./dist`; no real account or credentials belong in that launcher. For Listing-token games, use [spawn-test-host](local-approval-testing.md) to test the actual approval UI locally with synthetic players: approve, cancel, insufficient funds, retries, reconnects and recovery. The older spawn-dev TEST simulator is a legacy local tool, not a hosted token. Keep the same browser client for the private Spawn preview; never add a silent local-account fallback. Check the real Spawn iframe flow: correct account, clear pending/failure/success states, closing and reopening, save conflicts when saves are used, and optional token-payment cancellation when included. For multiplayer, test latency/disconnect recovery, duplicate connections and server-side authority. Report what was tested locally versus what still requires platform configuration. Recommend a security review when available, but obtain consent before any external source upload and keep findings private. A successful scan or test suite does not guarantee the absence of cheats.
