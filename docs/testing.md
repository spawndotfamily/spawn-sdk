...
For a clean regression: open the game, verify connected identity, submit a score and inspect `lastScore`; test payment cancellation and confirmation if used; rebuild, click Rebuild / reload and repeat. Upload only after these checks pass. Then wait for the automated malware check, play the same private preview and publish it only after the creator explicitly requests publication.
...
## Agent-readable local state

Testing should not require reading this folder end to end. `spawn-dev` exposes one compact JSON snapshot — read it instead of scraping the page:
...
worked example, and `testing/README.md` documents the harness API and its honest gaps. Scale it to
a fleet with `node testing/table-fleet.mjs --players 100` (or `runFleet()` from
`@spawndotfamily/sdk/testing/table-fleet.mjs`): N simulated players across many tables, the same
invariant asserted per table and in aggregate, and unfunded approvals refused and counted. Committed
