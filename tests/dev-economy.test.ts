...
const balances = (state: LocalTestState) => ['alice', 'bob', 'empty', 'creator', 'pool'].map(account => state.economy.balance(account));
...
test('confirmed entry credits the creator pool once, including retries', () => {
  ...
  assert.deepEqual(balances(state), [100, 100, 0, 1000, 0]);
  ...
  assert.deepEqual(balances(state), [90, 100, 0, 1000, 10]);
...
test('local creator funding, manual rewards and withdrawals conserve TEST balances', () => {
  ...
  state.economy.withdraw(30);
  assert.deepEqual(balances(state), [100, 120, 0, 980, 0]);
  ...
  assert.deepEqual(balances(new LocalTestState()), [100, 100, 0, 1000, 0]);
});
...
test('scores stay unverified for manual inspection without transferring rewards', () => {
  ...
  assert.deepEqual(balances(state), [100, 100, 0, 950, 50]);
